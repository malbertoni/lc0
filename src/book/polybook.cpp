...
bool PolyBook::init(const std::string& bookfile)
{
    if (bookfile.length() == 0) return false;
...
Move PolyBook::probe(Position& pos)
{
    ...
    m1 = pg_move_to_leela_move(pos, polyhash[idx1].move);
    ...
    if (idx1 == idx2)
        idx2 = index_first + 1;   
    Move  m2 = pg_move_to_leela_move(pos, polyhash[idx2].move);
...
Move PolyBook::probe(const std::string& fen, std::vector<std::string> &moves)
{
	Position pos;
...
Move PolyBook::pg_move_to_leela_move(const Position & pos, unsigned short pg_move)
{
    Move move = Move(pg_move);
